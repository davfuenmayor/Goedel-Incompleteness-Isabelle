...
theory embedding
  imports Main
begin
(** Configuration defaults for model finder nitpick *)
nitpick_params[assms=true, user_axioms=true, show_all, expect=genuine, format = 3] (*default settings*)
(*>*)

section \<open>Shallow Semantical Embedding SSE\<close>

subsection \<open>SSE of modal logic using a Kripke semantics\<close>

(**Note on notation: Isabelle/HOL operators are: @{text "\<and>, \<or>, \<longrightarrow>, \<longleftrightarrow>, \<not>"}*)
(**Important: we redefine Isabelle's symbol for classical negation @{text "\<not>"} to avoid confusions
with the notational conventions employed in the literature on paraconsistent logic.*)
abbreviation not::"bool\<Rightarrow>bool" ("\<sim>_"[52]53) where "\<sim>\<phi> \<equiv> \<not>\<phi>" 

typedecl w                  (**type for possible worlds*)
type_synonym wo = "w\<Rightarrow>bool" (**type for propositions (i.e. characteristic functions of sets of worlds)*)
...
abbreviation pneg::"wo\<Rightarrow>wo"      ("\<^bold>\<not>\<^sup>p_" [54]55)   where   "\<^bold>\<not>\<^sup>p\<phi>  \<equiv> \<phi>\<^sup>C \<^bold>\<union> (S\<^sub>\<not> \<phi>)"
...
(in whic case it will be indicated explicitly by indicating the option 'satisfy').
...
(** Unrestricted quantification for objects of arbitrary type:*)  
...
(** The following introduces some useful abbreviations (axiomatizes nothing!): *)
...
abbreviation "D_sem  \<equiv> \<forall>x. \<exists>y. x r y" (** i.e. seriality*)
abbreviation "T_sem  \<equiv> \<forall>x. x r x" (** i.e. reflexivity*)
abbreviation "IV_sem \<equiv> \<forall>x y z. x r y \<and> y r z \<longrightarrow> x r z" (**i.e. transitivity*)

(** The following verifies some useful Sahlqvist correspondences:*)
...
section \<open>Analysis of extensions of normal modal logics with GL axiom\<close>

subsection \<open>Characterising variants of axiom GL\<close>

(**Note that the symbols @{text "\<forall>, \<longrightarrow>"} are metalogical expressions belonging to Isabelle/HOL.
Also remember that the notation @{text " [\<^bold>\<turnstile> _ ]"} has been defined as modal validity (i.e. truth in all worlds).*)
abbreviation "GL_ax  \<equiv> \<forall>\<phi>. [\<^bold>\<turnstile> \<^bold>\<box>(\<^bold>\<box>\<phi>\<^bold>\<rightarrow>\<phi>) \<^bold>\<rightarrow> \<^bold>\<box>\<phi>]"
abbreviation "LR_rule  \<equiv> \<forall>\<phi>. [\<^bold>\<turnstile> \<^bold>\<box>\<phi>\<^bold>\<rightarrow>\<phi>] \<longrightarrow>  [\<^bold>\<turnstile> \<phi>]"

lemma "GL_ax \<longrightarrow> LR_rule" by blast
lemma "LR_rule \<longrightarrow> GL_ax" nitpick oops (*countermodel found*)

(**Note that the symbol @{text "\<Longrightarrow>"} below is the metalogical (Isabelle/HOL) turnstyle (equivalent to @{text "\<longrightarrow>"} for our purposes).*)

(** axiom GL and T are inconsistent!*)
lemma "\<lbrakk>GL_ax; T_ax\<rbrakk> \<Longrightarrow> False" by blast
(** rule LR and T are inconsistent!*)
lemma "\<lbrakk>LR_rule; T_ax\<rbrakk> \<Longrightarrow> False" by blast

(** axiom GL and D are also inconsistent!*)
lemma "\<lbrakk>GL_ax; D_ax\<rbrakk> \<Longrightarrow> False" by (metis bot2E)
(** rule LR and D are also inconsistent!*)
